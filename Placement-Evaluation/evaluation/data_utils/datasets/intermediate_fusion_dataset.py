...
class IntermediateFusionDataset(basedataset.BaseDataset):
    ...
    def __getitem__(self, idx):
        ...
        for cav_id, selected_cav_base in base_data_dict.items():
            ...
            selected_pose = selected_cav_base['params']['lidar_pose'] if 'lidar_pose' in selected_cav_base['params'] else selected_cav_base['params']['rsu']['cords']
            distance = math.sqrt((selected_pose[0] - ego_lidar_pose[0]) ** 2 +
                                 (selected_pose[1] - ego_lidar_pose[1]) ** 2)

            if distance > v2xvit.data_utils.datasets.COM_RANGE:
                continue

            selected_cav_processed, void_lidar = self.get_item_single_car(selected_cav_base, ego_lidar_pose, cav_id)
            ...
            # if selected_cav_base['ego'] or int(cav_id) <= 0:         # 测试一个setting，尽量提高带RSU后的精度
    ...
    def get_item_single_car(self, selected_cav_base, ego_pose, cav_id):
        """
        ...
        # velocity
        velocity = selected_cav_base['params']['ego_speed'] if 'ego_speed' in selected_cav_base['params'] else 0

...
class BaseDataset(Dataset):
    ...
    def __init__(self, params, visualize, train=True):
        ...
        if 'add_root_dir' in params:
            add_root_dir = params['add_root_dir'] if self.train else params['add_validate_dir']
            has_add = True if os.path.exists(add_root_dir) else False
            if has_add:
                print(f'Current dataset has additional dir: {add_root_dir}')
        else:
            print('Current dataset DOES NOT has additional file')
            add_root_dir = None
            has_add = False
        ...
        for (i, scenario_folder) in enumerate(scenario_folders):
            ...
            if has_add and os.path.exists(os.path.join(add_root_dir, scenario_name)):
                add_s_folder = os.path.join(add_root_dir, scenario_name)
                cav_list = cav_list + os.listdir(add_s_folder)
            ...
            if int(cav_list[0]) < 0:
                ...
                for idx, cav_id in enumerate(cav_list):
                    if int(cav_id) > 0:
                        cav_list = cav_list[idx:] + cav_list[:idx]
                        break

            assert int(cav_list[0]) >= 0
            ...
            for (j, cav_id) in enumerate(cav_list):
                ...
                cav_path = os.path.join(scenario_folder, cav_id) if int(cav_id) > 0 or not has_add \
                    else os.path.join(add_s_folder, cav_id)
                ...
                for timestamp in timestamps:
                    ...
                    yaml_file = os.path.join(cav_path, timestamp + '.yaml' if int(cav_id) > 0 else timestamp + '_additional.yaml')
    ...
    def retrieve_base_data(self, idx, cur_ego_pose_flag=True):
        ...
        for cav_id, cav_content in scenario_database.items():
            ...
            if timestamp_key not in cav_content or timestamp_key_delay not in cav_content:
                continue
            if not os.path.exists(cav_content[timestamp_key_delay]['lidar']):
                continue
            ...
            # 重要bug发现：雷达数据的坐标错了
            if int(cav_id) <= 0:
                data[cav_id]['lidar_np'][:, 0] = -data[cav_id]['lidar_np'][:, 0]
        return data
    ...
    def extract_timestamps(yaml_files):
        ...
        for file in yaml_files:
            ...
            timestamp = timestamp.replace('_additional', '')
    ...
    def calc_dist_to_ego(self, scenario_database, timestamp_key):
        ...
        for cav_id, cav_content in scenario_database.items():
            if cav_content['ego']:
                ...
                yaml_file = load_yaml(cav_content[timestamp_key]['yaml'])
                ego_lidar_pose = yaml_file['lidar_pose'] if 'lidar_pose' in yaml_file else yaml_file['rsu']['cords']
                break
        ...
        for cav_id, cav_content in scenario_database.items():
            if timestamp_key in cav_content:
                yaml_file = load_yaml(cav_content[timestamp_key]['yaml'])
                cur_lidar_pose = yaml_file['lidar_pose'] if 'lidar_pose' in yaml_file else yaml_file['rsu']['cords']
                distance = math.sqrt((cur_lidar_pose[0] -
                               ego_lidar_pose[0]) ** 2 +
                              (cur_lidar_pose[1] - ego_lidar_pose[1]) ** 2)
            else:
                distance = 1e10         # rsu does not have data in this timestep, it is far away from cav
    ...
    def reform_param(self, cav_content, ego_content, timestamp_cur,
        ...
        delay_cav_lidar_pose = delay_params['lidar_pose'] if 'lidar_pose' in delay_params else delay_params['rsu']['cords']
        delay_ego_lidar_pose = delay_ego_params["lidar_pose"] if 'lidar_pose' in delay_ego_params else delay_ego_params['rsu']['cords']

        cur_ego_lidar_pose = cur_ego_params['lidar_pose'] if 'lidar_pose' in cur_ego_params else cur_ego_params['rsu']['cords']
        cur_cav_lidar_pose = cur_params['lidar_pose'] if 'lidar_pose' in cur_params else cur_params['rsu']['cords']
        ...
        delay_params['vehicles'] = cur_params['vehicles'] if 'vehicles' in cur_params else {}

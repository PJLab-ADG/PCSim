...
#include "Carla/Sensor/mechanical.h" 
...
#include "ctime"
#include <cstdlib>
//extern FLidarDescription pandar64;
//extern FLidarDescription ruby128;
//extern FLidarDescription pandar128;
//extern FLidarDescription vlp16;
//extern FLidarDescription hdl64;
//extern FLidarDescription mems_m1;
//extern FLidarDescription pandar_qt;
//extern FLidarDescription bpearl;
//extern FLidarDescription pandar_xt;
//extern FLidarDescription pandar_40m;
//extern FLidarDescription pandar_40p;
//extern FLidarDescription vlp32;
...
void ARayCastSemanticLidar::Set(const FActorDescription &ActorDescription)
{
  ...
  if (created_lidar_type == "livox")  //livox式初始化  
  {
    for (int i = 0; i < 10; ++i) std::cout<<"creating "<<created_lidar_name<<std::endl;
	  LidarDescription = create_livox(created_lidar_name);
    LidarDescription.Range=LidarDescription.Range*100;
  }
  else if (created_lidar_type == "mems")  //mems式初始化
  {
    for (int i = 0; i < 10; ++i) std::cout<<"creating "<<created_lidar_name<<std::endl;
	  LidarDescription = create_mems(created_lidar_name);
    LidarDescription.Range=LidarDescription.Range*100;
  }
  else if (created_lidar_type == "mechanical")  //机械式初始化
  {
    for (int i = 0; i < 10; ++i) std::cout<<"creating "<<created_lidar_name<<std::endl;
	  LidarDescription = create_mechanical(created_lidar_name);
    LidarDescription.Range=LidarDescription.Range*100;
  }
...
void ARayCastSemanticLidar::CreateLasers()
{
  ...
  std::string my_lidar_type = TCHAR_TO_UTF8(*Description.LidarType);//FString to std::string
	...
	else if (Description.LidarType == "mechanical")
	{
    std::cout<<"creating lasers of "<<my_lidar_type<<std::endl;
    //ֱassign value use predifined vfov
		LaserAngles = Description.vfov;
	}
	else if (Description.LidarType == "mems")
	{
    std::cout<<"creating lasers of "<<my_lidar_type<<std::endl;
		//SimulateLidar with vfov and hfov
	}
	else if (Description.LidarType == "livox")
	{
    std::cout<<"creating lasers of "<<my_lidar_type<<std::endl;
		//read information from csv (done in livox.cpp)
	}
}
...
void ARayCastSemanticLidar::SimulateLidar(const float DeltaTime)
{
  ...
  if((Description.LidarType == "default") || (Description.LidarType == "mechanical"))
  {

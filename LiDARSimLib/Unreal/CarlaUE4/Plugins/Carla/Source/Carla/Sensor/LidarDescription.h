...
struct CARLA_API FLidarDescription
{
  ...
  FString LidarType = TEXT("default"); // zwq, lidar type(default,livox,mechanical,mems)

  UPROPERTY(EditAnywhere)
  FString NAME = TEXT("default"); // zwq, lidar name(modify int to string)
  /// Number of lasers.
  UPROPERTY(EditAnywhere)
  uint32 Channels = 32u;
  ...
  float Decay = 0.0f; //zwq,Point cloud density control parameter in liovx
  int LivoxSize = 0;
  std::vector<std::vector<float>> livox_csv_info; //zwq, save the csv data
  int LivoxCount = 0; //zwq, record the livox_csv_info index count

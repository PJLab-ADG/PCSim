...
void ARayCastSemanticLidar::CreateLasers()
{
  ...
  {
    std::cout << "creating lasers of " << my_lidar_type << std::endl;
    // ֱassign value use predefined vfov
      ...
      GetWorld()->GetPhysicsScene()->GetPxScene()->lockRead();
      {
        ...
        for (auto idxPtsOneLaser = 0u; idxPtsOneLaser < PointsToScanWithOneLaser; idxPtsOneLaser++) {
          ...
          if (PreprocessResult) {
            ...
            auto hited = ShootLaser(VertAngle, HorizAngle, HitResult, TraceParams, 0.0);
            if (hited)
              ...
              FRotator LaserRot(VertAngle, HorizAngle, 0); // float InPitch, float InYaw, float InRoll
              FRotator ResultRot = UKismetMathLibrary::ComposeRotators(
                  LaserRot,
                  LidarBodyRot);
              HitInfo.TraceStart = LidarBodyLoc;
              HitInfo.TraceEnd = Description.Range * UKismetMathLibrary::GetForwardVector(ResultRot) + LidarBodyLoc;
...
void ARayCastSemanticLidar::ComputeAndSaveDetections(const FTransform &SensorTransform)
{
  ...
  // todo get road spray particles data
  // get all particle emmiter & filter the range in 150
  // get all particles
  // do post process
...
bool ARayCastSemanticLidar::ShootLaser(const float VerticalAngle, const float HorizontalAngle, FHitResult &HitResult, FCollisionQueryParams &TraceParams, const float l_timestamp) const
{
  ...
  if (Description.EnableGhost && HitInfo.bBlockingHit) // enableghost and hit
  {
    ...
    ExitPoint = FirstHitPoint + HitResult.ImpactNormal * 0.1f; // 0.1 ???
